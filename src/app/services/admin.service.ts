...
export class AdminService {
  ...
  listenToAllRates() {
    ...
    this.unsubRates = onSnapshot(q, (snapshot) => {
  ...
  listenToAllOrders() {
    ...
    this.unsubOrders = onSnapshot(q, (snapshot) => {
      const orders: Order[] = [];
  ...
  async loadAllUsers() {
    ...
    merchantSnap.forEach((d) => users.push({ uid: d.id, role: 'merchant', profile: d.data() as MerchantProfile }));
    const farmerSnap = await getDocs(collection(db, 'farmers'));
    farmerSnap.forEach((d) => users.push({ uid: d.id, role: 'farmer', profile: d.data() as FarmerProfile }));
  ...
  async updateOrderStatus(orderId: string, newStatus: OrderStatus, note?: string) {
    ...
    snapshot.forEach((d) => { if (d.id === orderId) current = { id: d.id, ...d.data() } as Order; });
    if (!current) return;
